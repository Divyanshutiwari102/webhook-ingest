...
// Executor is a interface that wraps the Exec and QueryRow methods.
...
// InsertEvent stores the raw delivery and returns whether a new row was inserted.
func (s *Store) InsertEvent(ctx context.Context, ex Executor, e Event) (bool, error) {
	result, err := ex.Exec(ctx,
...
func (s *Store) UpsertCall(ctx context.Context, ex Executor, e Event) error {
	_, err := ex.Exec(ctx,
...
func (s *Store) IncrementAccountStats(ctx context.Context, ex Executor, accountID string, durationSec int) error {
	_, err := ex.Exec(ctx,
